 ...
 *
 * CVS/RCS keyword:
 * $Header: /home/thompson/cvs/dscan/seqset.c,v 2.0 2003/11/20 15:13:55 palumbo Exp $
 *
 */
...
long     count_seqset_entities(FILE *fptr,ss_type P, long nsize[])
{
	...
	while((c=fgetc(fptr))!=EOF){ if(c=='>') break; }
		...
		if(i >= MAX_NUMBER_SEQS) 
	   	...
	   	P->max_leng = MAX(long,P->max_leng,length);
		P->min_leng = MIN(long,P->min_leng,length);
		nsize[i] = length;
	}
	i--;
        return i;
