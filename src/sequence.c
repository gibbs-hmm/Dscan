 ...
 *
 * CVS/RCS keyword:
 * $Header: /home/thompson/cvs/dscan/sequence.c,v 2.0 2003/11/20 15:13:57 palumbo Exp $
 *
 */
...
e_type	ReadSeq(FILE *fptr, long I, long size, a_type A)
{
	...
	NEW(id,80,char);
        for(i=0, j = 0; (c=fgetc(fptr))!=EOF; i++){ 
		if(c=='\n') { if(i<80)id[i]= '\0'; break; }
		else if(i<79) {id[i] = c; j = i;}
		else if(i==79) id[i] = '\0';
	}
	if( id[j]=='R' && id[j-1]==' ' )
	        reverse=TRUE;
	...
	E->info=id;  E->n = length;        /* BT 9/26/97 */
	E->I = I;                          /* BT 9/27/97 */
...
long	GetFastaInfo(char *DBS_NAME, long max, long **pcounts, 
	long **psize, a_type A)                                    /* BT 9/26/97 */
/*****************************************************************
	...
	long flank, a_type A)
{
	...
	e = MIN(long,e,LenSeq(E));
	if (E->rev==TRUE)
		fprintf(fptr," %4ld",E->n-start+1-length+1);
	else
		fprintf(fptr,"%4ld  ",e);
}
...
{
	long	v,i,j,k,s,n,r,n1,n2,score=0,item,b,e,start,end,w=40;
	char	*seq1,*seq2,*str;
	long	center;
